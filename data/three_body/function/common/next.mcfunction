...
execute if score #GLOBAL visited_polar matches 1.. run return run function three_body:dawn/move
execute if score #GLOBAL visited_dried matches 1.. if score #GLOBAL visited_frozen matches 1.. if score #GLOBAL visited_overworld matches 1.. run return run function three_body:polarnight/move
...
execute as @e[type=marker,tag=next_dim_selector,tag=!current,sort=random,limit=1] run function three_body:common/run_move
