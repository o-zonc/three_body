execute store result score #mob_restore_value tmp run function product/overworld/mob/value/regen_value
...
function product/convert_upgrade_cost_to_text_named {id:"mob_regen",insertion:", "}
...
execute if score #trial_active trial matches 1 if score #trial_id trial matches 5 unless score #creation_available_mob_regen trial matches 1 run data modify storage data tmp.creation_ui.mob_regen set value {text:"[ 손실됨 ]",color:"dark_gray",shadow_color:-1428043265,bold:true,hover_event:{action:"show_text",value:{text:"그것은 손실되었습니다.",color:"dark_gray",shadow_color:-1428043265,bold:true}}}
tellraw @s ["", \
  { text: "  §6§l[ 몹 ]§r" },\
  { text: "\n" },\
