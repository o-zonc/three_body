...
$execute if data storage three_body:resource config.$(type).regen[{lvl:$(lvl)}] run data modify storage three_body:resource temp.cost set from storage three_body:resource config.$(type).regen[{lvl:$(lvl)}].cost
