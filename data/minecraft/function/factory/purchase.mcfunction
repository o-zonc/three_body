...
$execute unless entity @a[tag=accelerator_experiment_running] if score #$(id) factory_unlocked matches 1 if score #$(id) factory_level matches 6.. run title @s actionbar {text:"이미 최대 레벨입니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #factory_cost_check tmp matches 1 run title @s actionbar {text:"공장 건설·업그레이드 재료가 부족합니다.",color:"red",italic:true}
