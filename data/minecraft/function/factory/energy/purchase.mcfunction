...
execute unless entity @a[tag=accelerator_experiment_running] unless data storage data tmp.cost run title @s actionbar {text:"태양광 발전 연구가 완료되었습니다.",color:"green",italic:false}
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #factory_energy_cost_ok tmp matches 1 run title @s actionbar {text:"연구 재료가 부족합니다.",color:"red",italic:true}
