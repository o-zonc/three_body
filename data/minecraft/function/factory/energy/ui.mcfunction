...
execute if score #factory_energy_level upgrade matches 1 run scoreboard players set #factory_energy_reduction tmp 30
execute if score #factory_energy_level upgrade matches 2.. run scoreboard players set #factory_energy_reduction tmp 70
...
function util/blank
tellraw @s ["",{text:"  [ 태양광 발전 연구 ]",color:"yellow",bold:true},{text:"\n\n  공장의 석탄 의존도를 낮추고 실험에 필요한 에너지를 효율화합니다.",color:"gray",bold:false},{text:"\n  • 연구 단계: ",color:"gray"},{score:{name:"#factory_energy_level",objective:"upgrade"},color:"white"},{text:" / 3",color:"gray"},{text:"\n  • 공장 석탄 소비 감소: ",color:"gray"},{score:{name:"#factory_energy_reduction",objective:"tmp"},color:"yellow"},{text:"%",color:"yellow"},{text:"\n  • 입자가속기 실험 비용: ",color:"gray"},{storage:"data",nbt:"tmp.factory_energy.experiment",interpret:true},{text:"\n\n  Lv.1  석탄 소비 30% 감소",color:"dark_gray"},{text:"\n  Lv.2  석탄 소비 70% 감소",color:"dark_gray"},{text:"\n  Lv.3  입자가속기 실험 비용 50% 감소",color:"dark_gray"},{text:"\n\n  "},{storage:"data",nbt:"tmp.factory_energy.button",interpret:true},{text:"\n"}]
