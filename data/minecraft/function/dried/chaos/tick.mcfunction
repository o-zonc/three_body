# 메마른 세계 난세기 환경 효과
# 난세기 동안 실외 플레이어는 계속 불타며 실제 화염 피해를 받는다.
# 냉각 물약을 마시면 현재 난세기 동안 발화 효과 자체를 무시한다.

# 엔딩 중에는 화염 타이머와 보호 상태를 그대로 보존한다.
execute if entity @a[tag=ending_active] run return 0

# 냉각 물약을 마신 뒤 남은 유리병 1개를 회수하고 보호 태그를 부여한다.
...
# 난세기가 끝나면 보호 상태와 화염 피해 타이머를 정리한다.
...
# 플레이어의 Fire NBT는 명령으로 수정할 수 없으므로 화염 입자와 on_fire 피해로 구현한다.
# 하늘이 직접 보이는 보호받지 않은 실외 플레이어에게만 1초마다 피해를 준다.
...
# 실내로 들어가거나 냉각 물약으로 보호받으면 화염 효과가 적용되지 않는다.
