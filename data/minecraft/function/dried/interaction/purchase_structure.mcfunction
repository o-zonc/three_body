# 매크로 인수: id, name, color, x, y, z, cost
# 구버전 저장 상태를 unlock 스코어로 승계한다.
$execute in dried if data block $(x) $(y) $(z) {name:"$(id)"} run scoreboard players set #dried_$(id) unlock 1

$execute unless entity @a[tag=accelerator_experiment_running] if score #dried_$(id) unlock matches 1 run title @s actionbar {text:"$(name)은(는) 이미 해금했습니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #dried_structure_cost_ok tmp matches 1 run title @s actionbar {text:"시설 해금 재료가 부족합니다.",color:"red",italic:true}
