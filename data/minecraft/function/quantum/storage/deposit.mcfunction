...
execute unless entity @a[tag=accelerator_experiment_running] unless score #broken_quantum_deposit tmp matches 1.. run title @s actionbar {text:"저장할 깨진 양자 얽힘 파편이 없습니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] run title @s actionbar [{text:"",italic:false},{score:{name:"#broken_quantum_deposit",objective:"tmp"},color:"white"},{text:"개의 깨진 양자 얽힘 파편을 저장했습니다.",color:"aqua"}]
