...
execute unless entity @a[tag=accelerator_experiment_running] unless score #broken_quantum_storage var matches 1.. run title @s actionbar {text:"저장된 깨진 양자 얽힘 파편이 없습니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] run title @s actionbar [{text:"",italic:false},{score:{name:"#broken_quantum_withdraw",objective:"tmp"},color:"white"},{text:"개의 깨진 양자 얽힘 파편을 인출했습니다.",color:"light_purple"}]
