...
execute unless entity @a[tag=accelerator_experiment_running] unless score #overworld civilization_age matches 8.. run title @s actionbar {"text":"현대 시대에 도달해야 3층을 해금할 수 있습니다.","color":"red",italic:true}
execute unless score #overworld civilization_age matches 8.. run return 0
execute unless entity @a[tag=accelerator_experiment_running] unless score #GLOBAL elevator_unlocked matches 1 run title @s actionbar {"text":"엘리베이터 Lv. 1을 먼저 해금해야 합니다.","color":"red",italic:true}
