...
# 타임머신이 건설된 Lv.1부터 세대에 맞춰 표시한다.

# 기존 월드에 남은 중복 마커를 한 번 정리한다. 궤도 청크는 상시 로드된다.
...
# 시각 효과의 활성 여부는 타임머신 레벨만 기준으로 판단한다.
# 전역 차원 상태나 구조물 블록 NBT가 실제 상태와 어긋나도 마커 생성을 막지 않는다.

# 마커 총수·현재 레벨 설정 태그·방향 태그 수를 검사한다.
# 하나라도 어긋나면 기존 마커를 전부 지우는 레벨별 setup으로 즉시 재구성한다.
...
# 파티클은 마커 이동과 함께 매 틱 생성한다.
# 건물 주변 48블록의 플레이어에게 파티클 설정과 관계없이 보낸다.
...
# 이동 자체는 매틱 0.25블록으로 유지해 궤도 속도와 레벨별 연출 타이밍은 바꾸지 않는다.
...
# 각 변을 64틱 이동하면 다음 방향으로 전환한다. (0.25 × 64 = 16블록)
