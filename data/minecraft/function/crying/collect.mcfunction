# 우는 흑요석 수집
# 이미 수집한 조각은 네 수집 플래그에서 계산한 진행도만 회색으로 표시합니다.
...
$execute if score #crying_$(id) var matches 1 run title @s actionbar [{text:"",italic:false},{"score":{"name":"#crying_count","objective":"var"},"color":"dark_gray"},{"text":" / 4","color":"dark_gray"}]
$execute if score #crying_$(id) var matches 1 run return 0

# 0은 미수집, 1은 수집입니다. 해당 세계의 플래그만 한 번 변경합니다.
...
# 방금 처음 수집한 경우에만 진행 숫자를 보라색으로 표시합니다.
