...
$execute unless score #crying_$(id) var = #crying_$(id) var run scoreboard players set #crying_$(id) var 0

# 이미 수집한 조각이면 현재 진행도만 다시 표시합니다.
$execute if score #crying_$(id) var matches 1 run title @s actionbar [{"score":{"name":"#crying_count","objective":"var"}},{"text":" / 4"}]
...
title @s actionbar [{"score":{"name":"#crying_count","objective":"var"}},{"text":" / 4"}]
