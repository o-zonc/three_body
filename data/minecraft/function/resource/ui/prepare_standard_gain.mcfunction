...
$data modify storage data tmp.advancement_reward_ui.$(id) set value [{text:" → "},{score:{name:"#$(id)_gain",objective:"tmp"},color:"$(color)"},{text:"§7개"},{text:" ★",color:"$(color)",hover_event:{action:"show_text",value:[]}}]
$data modify storage data tmp.advancement_reward_ui.$(id)[3].hover_event.value set from storage data tmp.resource_gain_hover.$(id)
