# 매크로 인수: id, unlock, name, color, trigger, unit
$execute unless entity @a[tag=accelerator_experiment_running] unless score #$(unlock) unlock matches 1 run title @s actionbar {text:"해금되지 않은 자원입니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] unless data storage data tmp.cost run title @s actionbar {text:"더 이상 업그레이드할 수 없습니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #material_shop_cost_check tmp matches 1 run title @s actionbar {text:"재료가 부족합니다.",color:"red",italic:true}
