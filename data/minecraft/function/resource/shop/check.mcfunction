...
execute if score #coal unlock matches 1 run data modify storage data tmp.resource_shop.lines append value {text:"  • 석탄",color:"dark_gray",extra:[{text:"  수급량 ",color:"gray"},{score:{name:"#coal_shop_value",objective:"tmp"},color:"white"},{text:"배  (Lv. ",color:"gray"},{score:{name:"#coal_lvl",objective:"material_shop"},color:"white"},{text:")\n",color:"gray"}],bold:false}
