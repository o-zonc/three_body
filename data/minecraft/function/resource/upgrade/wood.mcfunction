# 독립 자원 시스템의 나무 재생 업그레이드
data remove storage data tmp.cost
execute unless score #wood unlock matches 1 run function resource/effect/failure
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #wood_regen_upgrade_cost_check tmp matches 1 run title @s actionbar {text:"재료가 부족합니다.",color:"red",italic:true}
...
scoreboard players operation #poss_effective_lvl tmp = #wood_regen_lvl upgrade
execute in overworld run playsound entity.ender_eye.death weather @a[tag=player] 0.5 -58.5 35.5 1 0.8
execute in overworld run playsound block.respawn_anchor.charge weather @a[tag=player] 0.5 -58.5 35.5 1 2
execute in overworld run particle end_rod 0.5 -58.5 35.5 0.3 0.3 0.3 0.4 30 force @a[tag=player]
execute as @a[tag=player] run function resource/material/wood/ui/info
return 1
