...
# 철 광석 파괴
execute if block -19 1 3 air unless score #iron_remain generate = #iron_remain generate run function resource/material/iron/regen/break

# 쿨다운이 전부 다 돌면 철 광석 설치
execute if block -19 1 3 air if score #iron_remain generate matches ..0 run function resource/material/iron/regen/place
