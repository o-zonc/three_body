...
execute if score #diamond_second_bonus_ui tmp matches 1.. run data modify storage data tmp.second_floor_ui.diamond set value [{text:"\n\n자원 상점 2층: §a수급량 +"},{score:{name:"#diamond_second_bonus_ui",objective:"tmp"},color:"green"}]

data modify storage data tmp.first_floor_ui.diamond set value {text:""}
execute if score #diamond_shop_multiplier_ui tmp matches 2.. run data modify storage data tmp.first_floor_ui.diamond set value [{text:"\n자원 상점 1층: §a수급량 ×"},{storage:"data",nbt:"tmp.resource_shop.diamond.now.value",color:"green",plain:true}]
...
execute unless score #diamond_gain tmp = #diamond_base_gain tmp run data modify storage data tmp.advancement_reward_ui.diamond set value [{text:" → "},{score:{name:"#diamond_gain",objective:"tmp"},color:"green"},{text:"§7개"},{text:" ★",color:"green",hover_event:{action:"show_text",value:[{text:"§a최종 자원 수급량§r§7이 증가합니다."},{storage:"data",nbt:"tmp.second_floor_ui.diamond",interpret:true},{storage:"data",nbt:"tmp.first_floor_ui.diamond",interpret:true},{storage:"data",nbt:"tmp.frozen_advancement_ui.diamond",interpret:true}]}}]
