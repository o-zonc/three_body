...
execute if entity @a[nbt={Dimension:"minecraft:frozen"}] if entity @a[advancements={1_frozen/01_chaos_survivor=true}] run data modify storage data tmp.resource_gain_hover.cold append value {text:"\n발전과제 수급량: ×",color:"gray"}
execute if entity @a[nbt={Dimension:"minecraft:frozen"}] if entity @a[advancements={1_frozen/01_chaos_survivor=true}] run data modify storage data tmp.resource_gain_hover.cold append value {text:"10",color:"gold"}
data modify storage data tmp.resource_gain_hover.cold append from storage data tmp.dawn_amplifier_ui
data modify storage data tmp.advancement_reward_ui.cold set value [{text:" → "},{score:{name:"#cold_gain",objective:"tmp"},color:"aqua"},{text:"§7개"},{text:" ★",color:"aqua",hover_event:{action:"show_text",value:[]}}]
data modify storage data tmp.advancement_reward_ui.cold[3].hover_event.value set from storage data tmp.resource_gain_hover.cold
