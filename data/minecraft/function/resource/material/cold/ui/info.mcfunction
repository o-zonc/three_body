...
execute unless score #cold_gain tmp = #cold_base_gain tmp run data modify storage data tmp.advancement_reward_ui.cold set value [{text:" → "},{score:{name:"#cold_gain",objective:"tmp"},color:"green"},{text:"§7개"},{text:" ★",color:"green",hover_event:{action:"show_text",value:[{text:"§a최종 자원 수급량§r§7이 증가합니다."},{storage:"data",nbt:"tmp.second_floor_ui.cold",interpret:true},{storage:"data",nbt:"tmp.frozen_advancement_ui.cold",interpret:true}]}}]
