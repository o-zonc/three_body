...
execute unless score #information_bank_unlocked meta matches 1.. if score #information_bank_visible tmp matches 1001.. run scoreboard players set #information_bank_visible tmp 1000
data modify storage data tmp.information_bank_limit set value {text:" / 1000조각",color:"gray"}
execute if score #information_bank_unlocked meta matches 1.. run data modify storage data tmp.information_bank_limit set value {text:"조각 (상한 해제)",color:"gray"}
