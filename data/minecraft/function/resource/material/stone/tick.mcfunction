# 돌은 참고 구현처럼 "노드 파괴 감지"와 "나무 채취 횟수 기반 재생성"을 분리합니다.
# remain 값은 tick 쿨타임이 아니라 재설치까지 필요한 나무 채취 횟수입니다.
scoreboard players set #stone_harvested_this_tick var 0

# 잠긴 동안 이전 문명의 돌 블록이 물리적으로 남지 않게 합니다.
...
# 생산 정지 중에는 이전 자동 채굴 신호를 보존하지 않습니다.
...
# 노드 위치에 블록이 있으면 idle 상태입니다. remain은 블록이 실제로 사라진 뒤에만 의미를 가집니다.
...
# air + remain 미존재를 단일 파괴 edge로 사용합니다.
# 채굴 통계는 수확 자체를 막는 조건이 아니라 카탈리스트 직접 채굴 판정에만 사용합니다.
...
# 감소는 wood/harvest -> stone/count에서만 수행합니다.
# 0회 단계이거나 이미 필요한 나무 채취 횟수를 모두 채운 상태에서 재설치가 점유 때문에 실패했다면 매 tick 재시도합니다.
