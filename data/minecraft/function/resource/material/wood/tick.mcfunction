# 참고 구현의 핵심 상태 머신을 유지한다.
# - 노드가 존재하면 remain 점수를 비워 idle 상태로 둔다.
# - 노드가 air이고 remain 점수가 없으면 방금 파괴된 것으로 보고 정확히 한 번 수확한다.
# - remain의 존재 여부만 채굴 edge와 재생 대기 상태의 기준으로 사용한다.

# restart/정산 등으로 잠긴 동안 이전 노드가 물리적으로 남지 않게 한다.
...
# 생산 정지 중에는 이전 자동 채굴 신호를 다음 활성화 시점까지 끌고 가지 않는다.
...
# 노드 위치에 블록이 있으면 아직 채굴되지 않은 상태이다.
# 플레이어가 재생성 위치를 점유해 다른 블록이 있는 경우도 덮어쓰지 않는다.
...
# air + remain 미존재가 이번 채굴의 단일 edge 신호이다.
