...
execute positioned -76.5 1.9375 13.5 unless entity @e[tag=blaze,distance=..0.2,type=item_display] unless score #blaze_remain generate = #blaze_remain generate run function resource/nether/blaze/regen/summon

# 쿨다운이 전부 다 돌면 블레이즈 소환
execute if score #blaze_remain generate matches ..0 run function resource/nether/blaze/regen/summon
