...
# 업그레이드 재료 소지 여부 확인
execute store result score #blaze_drop_upgrade_cost_check tmp run function resource/check_cost

# 소지 시 업그레이드
execute if score #blaze_drop_upgrade_cost_check tmp matches 1 run function resource/take_upgrade_cost
...
execute if score #blaze_drop_upgrade_cost_check tmp matches 1 as @a[tag=player] run function resource/nether/blaze/ui/info
