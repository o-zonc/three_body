...
# 석탄 광석 파괴
execute if block -15 1 3 air unless score #coal_remain generate = #coal_remain generate run function resource/overworld/coal/regen/break

# 쿨다운이 전부 다 돌면 석탄 광석 설치
execute if block -15 1 3 air if score #coal_remain generate matches ..0 run function resource/overworld/coal/regen/place
