execute store result score #iron_restore_cooldown tmp run function resource/overworld/iron/value/regen_value
...
execute if score #trial_active trial matches 1 if score #trial_id trial matches 5 unless score #creation_available_iron_resourceion trial matches 1 run data modify storage data tmp.iron_restore_component set value {text:"[ 손실됨 ]",color:"dark_gray",shadow_color:-1428043265,bold:true,hover_event:{action:"show_text",value:{text:"그것은 손실되었습니다.",color:"dark_gray",shadow_color:-1428043265,bold:true}}}
