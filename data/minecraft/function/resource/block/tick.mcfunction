...
# 노드 위치가 air가 아니면 아직 채굴되지 않은 idle 상태입니다.
# 지정 자원 블록이 아닌 다른 블록이 있어도 재생성으로 덮어쓰지 않습니다.
...
# harvest를 처리한 tick에는 쿨다운을 차감하지 않아 설정값만큼 완전한 대기 tick을 보장합니다.
