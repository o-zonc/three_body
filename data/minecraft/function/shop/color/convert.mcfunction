# 매크로 인수: source, source_name, source_color, target, target_name, target_color, base, amount
execute unless score #color_resources_unlocked var matches 1 run return 0
$execute unless entity @a[tag=accelerator_experiment_running] unless score #$(source) material matches 1.. run title @s actionbar {text:"$(source_name) 자원이 부족합니다.",color:"red",bold:false,italic:true}
...
$execute unless entity @a[tag=accelerator_experiment_running] unless score #$(source) material >= #color_conversion_source_amount tmp run title @s actionbar [{text:"",italic:true},{text:"$(source_name) 자원이 부족합니다. ",color:"red",bold:false},{text:"필요: ",color:"red",bold:false},{score:{name:"#color_conversion_source_amount",objective:"tmp"},color:"red",bold:false},{text:"개",color:"red",bold:false}]
