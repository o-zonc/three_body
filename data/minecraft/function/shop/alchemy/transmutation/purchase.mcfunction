# Macro args: source, source_name, source_amount, target, target_name, target_color, cooldown
...
$execute if score $(cooldown) var matches 1.. run title @s actionbar [{text:"다시 연성하려면 ",color:"red"},{score:{name:"$(cooldown)",objective:"var"},color:"yellow"},{text:"틱 기다려야 합니다.",color:"red"}]
...
$execute unless score #alchemy_transmutation_cost_ok tmp matches 1 run title @s actionbar {text:"$(source_name) 자원이 부족합니다.",color:"red"}
...
$title @s actionbar [{text:"$(source_name) $(source_amount)개를 ",color:"gray"},{text:"$(target_name) 1개",color:"$(target_color)"},{text:"로 연성했습니다.",color:"gray"}]
