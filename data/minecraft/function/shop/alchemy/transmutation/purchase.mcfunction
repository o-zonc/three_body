# 매크로 인수: source, source_name, source_amount, target, target_name, target_color, cooldown, lapis_amount
execute unless score #alchemy_workshop unlock matches 1 run return 0
execute unless entity @a[tag=accelerator_experiment_running] unless score #level alchemy_workshop matches 1.. run title @s actionbar {text:"연금술 공방 Lv. 1이 필요합니다.",color:"red",italic:true}
execute unless score #level alchemy_workshop matches 1.. run return 0

$execute unless entity @a[tag=accelerator_experiment_running] if score $(cooldown) var matches 1.. run title @s actionbar [{text:"",italic:true},{text:"다시 연성하려면 ",color:"red"},{score:{name:"$(cooldown)",objective:"var"},color:"yellow"},{text:"틱 기다려야 합니다.",color:"red"}]
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #alchemy_transmutation_cost_ok tmp matches 1 run title @s actionbar {text:"연성에 필요한 자원이 부족합니다.",color:"red",italic:true}
...
$execute unless entity @a[tag=accelerator_experiment_running] run title @s actionbar [{text:"",italic:false},{text:"$(source_name) $(source_amount)개를 ",color:"gray"},{text:"$(target_name) 1개",color:"$(target_color)"},{text:"로 연성했습니다.",color:"gray"}]
