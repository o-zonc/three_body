# 연금술 물약 런타임
# potion_used 통계와 직전 틱에 손에 들고 있던 커스텀 물약 태그를 조합해 섭취를 감지한다.

# 기존 세이브에서도 첫 소비를 놓치지 않도록 누락된 플레이어 통계 기준값을 보정한다.
...
# 구매 쿨타임은 era_paused 동안, 활성 카탈리스트 지속시간은 차원 이동 연출 중에만 멈춘다.
...
# 카탈리스트가 끝난 플레이어의 임시 효과와 채굴 속도 배율을 원래 값으로 돌린다.
...
# 수정 전 카탈리스트가 남긴 성급함 효과도 종료 시 함께 정리한다.
...
# 직전 틱에 추적한 물약을 실제로 마셨을 때만 각 효과를 실행한다.
...
# 다음 섭취를 판정하기 위해 현재 손의 커스텀 물약을 기록한다.
# 주 손에 일반 물약이 있으면 보조 손 커스텀 물약을 잘못 소비한 것으로 판정하지 않도록 주 손을 우선한다.
