...
execute unless entity @a[tag=accelerator_experiment_running] unless data storage data tmp.alchemy_shop.now run title @s actionbar {text:"더 이상 업그레이드할 수 없습니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #overworld civilization_age >= #alchemy_required_age tmp run title @s actionbar [{text:"",italic:true},{text:"문명 단계가 부족합니다. ",color:"red"},{text:"(필요 단계: ",color:"gray"},{score:{name:"#alchemy_required_age",objective:"tmp"},color:"white"},{text:")",color:"gray"}]
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #alchemy_shop_cost_check tmp matches 1 run title @s actionbar {text:"재료가 부족합니다.",color:"red",italic:true}
