...
execute unless data storage data tmp.alchemy_shop.now run title @s actionbar "§c더 이상 업그레이드할 수 없습니다."
...
execute unless score #overworld civilization_age >= #alchemy_required_age tmp run title @s actionbar ["",{text:"문명 단계가 부족합니다. ",color:"red"},{text:"(필요 단계: ",color:"gray"},{score:{name:"#alchemy_required_age",objective:"tmp"},color:"white"},{text:")",color:"gray"}]
...
execute unless score #alchemy_shop_cost_check tmp matches 1 run title @s actionbar "§c재료가 부족합니다."
