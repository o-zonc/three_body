...
execute if score #alchemy_lab_level upgrade matches 15.. run title @s actionbar {"text":"모든 연구가 완료되었습니다.","color":"green",italic:false}
...
execute unless score #alchemy_lab_can_pay tmp matches 1 run title @s actionbar {"text":"연구에 필요한 자원이 부족합니다.","color":"red",italic:true}
...
title @s actionbar [{text:"",italic:false},{"text":"마법 연구 완료 — Lv. ","color":"light_purple"},{"score":{"name":"#alchemy_lab_level","objective":"upgrade"},"color":"white"}]
