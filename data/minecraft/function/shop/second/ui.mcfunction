...
$execute if score #$(id)_second_kind tmp matches 1 run data modify storage data tmp.second_shop.amount set value {text:"\n  수급량: 1(",color:"gray",extra:[{text:"+",color:"green"},{score:{name:"#second_common_bonus",objective:"tmp"},color:"green"},{text:"+",color:"light_purple"},{score:{name:"#second_extreme_multiplier",objective:"tmp"},color:"light_purple"},{text:")개 ",color:"gray"},{text:"⁂",color:"light_purple",hover_event:{action:"show_text",value:["",{text:"수급량 구성\n\n",color:"gray"},{text:"모든 자원 생산량 증가: +",color:"green"},{score:{name:"#second_common_bonus",objective:"tmp"},color:"green"},{text:"\n극한 환경 수급 증가: +",color:"light_purple"},{score:{name:"#second_extreme_multiplier",objective:"tmp"},color:"light_purple"}]}}]}
