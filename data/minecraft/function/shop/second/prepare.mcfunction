...
# 열기·냉기 단말은 보너스 대신 자연 지급 주기를 표시한다.
...
# 열기·냉기 자연 수급: 선택적 편의 강화이므로 금속 비용과 회수 가능한 소량의 해당 환경 자원만 요구한다.
...
# 기초 생산 공정: 2층의 선행 산업 기반. 보석 대신 오버월드 금속과 후반 연구 자원을 사용한다.
...
# 금·다이아몬드 생산: 높은 후반 자기 생산량 대신 연결 차원의 희귀 자원을 진행 게이트로 사용한다.
...
$execute if score #$(id)_second_gate tmp matches 1 if score #second_lvl tmp matches 1 run data modify storage data tmp.cost set value [{type:"iron",amount:550},{type:"heat",amount:15}]
$execute if score #$(id)_second_gate tmp matches 1 if score #second_lvl tmp matches 2 run data modify storage data tmp.cost set value [{type:"iron",amount:2200},{type:"heat",amount:50}]
$execute if score #$(id)_second_gate tmp matches 1 if score #second_lvl tmp matches 3 run data modify storage data tmp.cost set value [{type:"iron",amount:8800},{type:"heat",amount:150}]
...
$execute if score #$(id)_second_gate tmp matches 2 if score #second_lvl tmp matches 1 run data modify storage data tmp.cost set value [{type:"iron",amount:550},{type:"cold",amount:15}]
$execute if score #$(id)_second_gate tmp matches 2 if score #second_lvl tmp matches 2 run data modify storage data tmp.cost set value [{type:"iron",amount:2200},{type:"cold",amount:50}]
$execute if score #$(id)_second_gate tmp matches 2 if score #second_lvl tmp matches 3 run data modify storage data tmp.cost set value [{type:"iron",amount:8800},{type:"cold",amount:150}]

# 극한 환경 수급: 두 차원의 진행을 함께 요구하는 핵심 성장 강화이다.
...
$execute if score #$(id)_second_kind tmp matches 4 if score #second_lvl tmp matches 1 run data modify storage data tmp.cost set value [{type:"iron",amount:1000},{type:"heat",amount:15},{type:"cold",amount:15}]
$execute if score #$(id)_second_kind tmp matches 4 if score #second_lvl tmp matches 2 run data modify storage data tmp.cost set value [{type:"iron",amount:3500},{type:"heat",amount:45},{type:"cold",amount:45}]
$execute if score #$(id)_second_kind tmp matches 4 if score #second_lvl tmp matches 3 run data modify storage data tmp.cost set value [{type:"iron",amount:14000},{type:"heat",amount:120},{type:"cold",amount:120},{type:"information",amount:12},{type:"time",amount:6}]
