...
execute unless score #hole_claims var < #hole_max_claims tmp run title @s actionbar {text:"이번 문명에서 가능한 수급을 모두 마쳤습니다.",color:"dark_gray",italic:true}
execute unless score #hole_claims var < #hole_max_claims tmp run return 0

# 수급 횟수에 따라 정보/시간 비용이 급격히 증가합니다.
...
execute unless score #hole_cost_check tmp matches 1 run title @s actionbar {text:"정보 또는 시간이 부족합니다.",color:"red",italic:true}
...
title @s actionbar [{text:"",italic:false},{text:"흑요석 +1",color:"dark_gray",shadow_color:-8388480,bold:true},{text:"  ·  균열이 한층 불안정해졌습니다.",color:"dark_gray",bold:false}]
