...
execute unless entity @a[tag=accelerator_experiment_running] unless score #GLOBAL current_dim matches 3 run title @s actionbar {text:"극야 차원의 보관소에서만 사용할 수 있습니다.",color:"red",italic:true}
execute unless score #GLOBAL current_dim matches 3 run return 0
execute unless entity @a[tag=accelerator_experiment_running] unless score #GLOBAL reckoning_ready matches 1.. run title @s actionbar {text:"이번 문명의 정산이 이미 끝났습니다.",color:"red",italic:true}
execute unless score #GLOBAL reckoning_ready matches 1.. run return 0
execute unless entity @a[tag=accelerator_experiment_running] if score #spacetime_expansion_used var matches 1 run title @s actionbar {text:"이번 극야에서는 이미 시공간 확장을 사용했습니다.",color:"red",italic:true}
execute if score #spacetime_expansion_used var matches 1 run return 0

data remove storage data tmp.cost
data modify storage data tmp.cost set value [{type:"time",amount:1},{type:"no_obsidian",amount:1}]
execute store result score #spacetime_expansion_cost_check tmp run function resource/check_cost
execute unless entity @a[tag=accelerator_experiment_running] unless score #spacetime_expansion_cost_check tmp matches 1 run title @s actionbar {text:"시간 1조각이 필요합니다.",color:"red",italic:true}
...
function shop/third/storage_ui {id:"information",name:"정보",title:"보관소",color:"light_purple",unit:"조각",bank_max:2000,trigger:1821,withdraw_trigger:1825}
return 1
