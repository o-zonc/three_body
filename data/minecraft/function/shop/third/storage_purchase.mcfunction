...
execute unless entity @a[tag=accelerator_experiment_running] unless score #third_lvl tmp matches ..2 run title @s actionbar {text:"이미 최대 단계입니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #third_cost_check tmp matches 1 run title @s actionbar {text:"재료가 부족합니다.",color:"red",italic:true}
