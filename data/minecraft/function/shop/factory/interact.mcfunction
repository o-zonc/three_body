...
function resource/convert_cost_to_text_named {id:"factory_shop",insertion:", "}
execute store result score #factory_required_age tmp run data get storage data tmp.factory_shop.required_age

execute unless score #overworld civilization_age >= #factory_required_age tmp run tellraw @s ["",{text:"  [ 공장 건설 단말 ]",color:"gold",bold:true},{text:"\n\n  다음 시설: ",color:"gray"},{storage:"data",nbt:"tmp.factory_shop.name"},{text:"\n  현재 문명 단계에서는 건설할 수 없습니다.",color:"red"},{text:"\n  필요 문명 단계: ",color:"gray"},{score:{name:"#factory_required_age",objective:"tmp"},color:"white"},{text:"\n"}]
