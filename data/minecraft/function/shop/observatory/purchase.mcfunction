execute unless entity @a[tag=accelerator_experiment_running] if score #observatory unlock matches 1.. run title @s actionbar {"text":"관측소는 이미 구매했습니다.","color":"red",italic:true}
execute if score #observatory unlock matches 1.. run return 0

execute unless entity @a[tag=accelerator_experiment_running] unless score #overworld civilization_age matches 3.. run title @s actionbar {"text":"관측소가 잠겨 있습니다. 청동기 시대에 도달해야 합니다.","color":"red",italic:true}
