...
$execute unless entity @a[tag=accelerator_experiment_running] if score #$(id)_mover unlock matches 1.. run title @s actionbar {text:"이미 영구 구매한 아이템입니다.",color:"yellow",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] if score #item_purchase_cost_check tmp matches 1 run title @s actionbar {text:"아이템을 영구 구매했습니다.",color:"green",italic:false}
...
execute unless entity @a[tag=accelerator_experiment_running] run title @s actionbar {text:"재료가 부족합니다.",color:"red",italic:true}
return 0
