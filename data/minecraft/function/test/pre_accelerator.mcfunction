# 입자가속기 건설 직전의 정상적인 현대 진행 상태를 구성합니다.
# /function test/pre_accelerator

# 플레이어를 정상 플레이 위치로 복귀시킵니다.
...
# 공장은 건설 완료 상태이며, 현대 진입 조건인 네 자동화만 기본 단계로 해금합니다.
...
# 입자가속기·실험·고장·외계 자원·타임머신은 모두 시작 전 상태로 되돌립니다.
...
# 일반 자원은 Material 원장을 정확히 1,000,000으로 설정합니다.
...
# 메타 자원은 실제 아이템/보관 원장을 초기화한 뒤 기존 지급 API로 500을 넣습니다.
...
scoreboard players set #world_eye_shop_lvl upgrade 0
function meta/sync
...
scoreboard players set #material_add_value tmp 500
function resource/add_material/world_eye
function meta/sync
