# 신규 플레이어 또는 전체 restart 후 재초기화된 플레이어에게 한 번만 실행한다.
# /trigger objective는 사용 후 각 핸들러에서 해당 플레이어만 다시 활성화한다.
execute unless score #GLOBAL factory_build_stage = #GLOBAL factory_build_stage run function migration/factory_build_stage
...
# potion_used 통계는 첫 사용 전에는 score holder가 없을 수 있으므로 명시적으로 0을 만든다.
# restart 직후에는 현재 누적 통계를 기준점으로 삼아 이전 사용을 새 소비로 오인하지 않는다.
...
# 신규/재초기화 플레이어의 일반 발전과제 달성 상태를 한 번 동기화한다.
