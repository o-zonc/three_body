...
execute unless entity @a[tag=accelerator_experiment_running] unless score #withdraw_space tmp matches 1.. run title @s actionbar {text:"이미 소지 한도만큼 가지고 있습니다.",color:"red",italic:true}
execute unless score #withdraw_space tmp matches 1.. run return 0
$execute unless entity @a[tag=accelerator_experiment_running] unless score #$(id)_bank meta matches 1.. run title @s actionbar {text:"보관소가 비어 있습니다.",color:"red",italic:false}
...
$execute unless entity @a[tag=accelerator_experiment_running] run title @s actionbar [{text:"",italic:false},{text:"$(name) 보관소에서 ",color:"gray"},{text:"$(name) ", color:"$(color)"},{score:{name:"#withdraw_amount",objective:"tmp"},color:"$(color)"},{text:"조각",color:"$(color)"},{text:"을 인출했습니다.",color:"gray"}]
