...
# 정보와 시간이 모두 있어야 반응을 시작할 수 있습니다.
execute unless score #dawn_reactor_info_before tmp matches 1.. run title @s actionbar {text:"반응시킬 정보가 없습니다.",color:"red",italic:true}
execute unless score #dawn_reactor_info_before tmp matches 1.. run return 0
execute unless score #dawn_reactor_time_before tmp matches 1.. run title @s actionbar {text:"반응시킬 시간이 없습니다.",color:"red",italic:true}
...
function meta/information/give_item with storage data tmp.dawn_reactor_info_bonus
function meta/time/give_item with storage data tmp.dawn_reactor_time_bonus
