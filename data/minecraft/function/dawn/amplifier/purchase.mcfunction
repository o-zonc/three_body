...
execute unless entity @a[tag=accelerator_experiment_running] if score #GLOBAL dawn_reactor_purchased matches 1.. run title @s actionbar {text:"리액터의 시공간 반응이 시작되어 더 이상 증폭기를 조정할 수 없습니다.",color:"gray",italic:true}
...
$execute unless entity @a[tag=accelerator_experiment_running] if score #dawn_$(id)_amp meta matches 3.. run title @s actionbar {text:"이미 최대 단계입니다.",color:"gold",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #dawn_amp_cost_check tmp matches 1 run title @s actionbar {text:"재료가 부족합니다.",color:"red",italic:true}
...
# 각 증폭기를 해금하면(내부 meta=1) 해당 장치의 스테인드글라스를 채색한다.
$execute if score #dawn_$(id)_amp meta matches 1 run function dawn/amplifier/visual/$(id)
# 증폭기 발전과제와 리액터 보상은 기존처럼 Lv.1(내부 meta=2)에서 획득한다.
