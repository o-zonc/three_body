...
# 금 파괴
execute if block -74 1 -5 air unless score #gold_remain generate = #gold_remain generate run function product/nether/gold/regen/break

# 쿨다운이 전부 다 돌면 금 광석 설치
execute if block -74 1 -5 air if score #gold_remain generate matches ..0 run function product/nether/gold/regen/place
