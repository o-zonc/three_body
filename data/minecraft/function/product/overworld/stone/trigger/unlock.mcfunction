...
# 업그레이드 재료 소지 여부 확인
execute store result score #stone_unlock_cost_check tmp run function product/check_cost

# 소지 시 해금
execute if score #stone_unlock_cost_check tmp matches 1 run function product/take_cost
execute if score #stone_unlock_cost_check tmp matches 1 run scoreboard players set #stone unlock 1
execute if score #stone_unlock_cost_check tmp matches 1 run function product/overworld/stone/regen/place
