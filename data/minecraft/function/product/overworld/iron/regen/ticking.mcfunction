...
execute if score #trial_active trial matches 1 if score #trial_id trial matches 5 unless score #creation_available_iron_production trial matches 1 run setblock -19 1 3 air replace
execute if score #trial_active trial matches 1 if score #trial_id trial matches 5 unless score #creation_available_iron_production trial matches 1 run scoreboard players reset #iron_remain generate
...
# 철 광석 파괴
execute if block -19 1 3 air unless score #iron_remain generate = #iron_remain generate run function product/overworld/iron/regen/break

# 쿨다운이 전부 다 돌면 철 광석 설치
execute if block -19 1 3 air if score #iron_remain generate matches ..0 run function product/overworld/iron/regen/place
