...
# 업그레이드 재료 소지 여부 확인
execute store result score #mob_unlock_cost_check tmp run function product/check_cost

# 소지 시 해금
execute if score #mob_unlock_cost_check tmp matches 1 run function product/take_cost
execute if score #mob_unlock_cost_check tmp matches 1 run scoreboard players set #mob unlock 1
execute if score #mob_unlock_cost_check tmp matches 1 run function product/overworld/mob/regen/summon
