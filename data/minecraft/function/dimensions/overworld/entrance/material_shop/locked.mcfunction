...
execute if score #trial_active trial matches 1 if score #trial_id trial matches 5 unless score #creation_available_material_shop trial matches 1 run data modify storage data tmp.creation_ui.material_shop set value {text:"         ✶",color:"dark_gray",shadow_color:-1428043265,bold:true,hover_event:{action:"show_text",value:{text:"그것은 손실되었습니다.",color:"dark_gray",shadow_color:-1428043265,bold:true}}}
tellraw @s ["", \
  { text: "  §6§l< 구역 '자원 상점' >§r" },\
  { text: "\n\n" },\
