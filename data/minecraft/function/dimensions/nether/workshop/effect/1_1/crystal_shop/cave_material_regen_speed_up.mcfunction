...
execute if score #1_1_cost_check tmp matches 1 run scoreboard players add #cave_material_regen_speed_up_lvl crystal_shop 1
execute if score #1_1_cost_check tmp matches 1 run title @a[tag=player] actionbar ["", {text:"§6🍀 §3§l아차원 초월§7: 동굴 자원 재생산 속도 자동 업그레이드 §8(Lv. "}, {score:{name:"#cave_material_regen_speed_up_lvl",objective:"crystal_shop"}, color:"aqua"}, {text:"§8)"}]
