...
execute if score #1_1_cost_check tmp matches 1 run scoreboard players add #iron_lvl material_shop 1
execute if score #1_1_cost_check tmp matches 1 run title @a[tag=player] actionbar ["", {text:"§6🍀 §3§l아차원 초월§7: 철 수급량 자동 업그레이드 §8(Lv. "}, {score:{name:"#iron_lvl",objective:"material_shop"}, color:"aqua"}, {text:"§8)"}]
