...
execute if score #1_1_cost_check tmp matches 1 run scoreboard players add #speed_up_lvl piglin_head 1
execute if score #1_1_cost_check tmp matches 1 run title @a[tag=player] actionbar ["", {text:"§6🍀 §3§l아차원 초월§7: 피글린 머리 작동 시간 자동 업그레이드 §8(Lv. "}, {score:{name:"#speed_up_lvl",objective:"piglin_head"}, color:"aqua"}, {text:"§8)"}]
...
execute if score #1_1_cost_check tmp matches 1 run function dimensions/nether/piglin_head/reset_remain