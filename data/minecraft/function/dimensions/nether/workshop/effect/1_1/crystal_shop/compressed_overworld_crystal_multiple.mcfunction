...
execute if score #1_1_cost_check tmp matches 1 run scoreboard players add #compressed_overworld_crystal_multiple_lvl crystal_shop 1
execute if score #1_1_cost_check tmp matches 1 run title @a[tag=player] actionbar ["", {text:"§6🍀 §3§l아차원 초월§7: 오버월드 결정 획득량 자동 업그레이드 §8(Lv. "}, {score:{name:"#compressed_overworld_crystal_multiple_lvl",objective:"crystal_shop"}, color:"aqua"}, {text:"§8)"}]
