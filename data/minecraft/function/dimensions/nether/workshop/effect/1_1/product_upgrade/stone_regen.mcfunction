...
execute if score #1_1_cost_check tmp matches 1 run scoreboard players add #stone_regen_lvl upgrade 1
execute if score #1_1_cost_check tmp matches 1 run title @a[tag=player] actionbar ["", {text:"§6🍀 §3§l아차원 초월§7: 돌 재생산 속도 자동 업그레이드 §8(Lv. "}, {score:{name:"#stone_regen_lvl",objective:"upgrade"}, color:"aqua"}, {text:"§8)"}]
