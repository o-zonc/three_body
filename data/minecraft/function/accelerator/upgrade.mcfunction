execute unless entity @a[tag=accelerator_experiment_running] if entity @s[tag=accelerator_experiment_running] run title @s actionbar {text:"실험이 진행 중일 때는 입자가속기를 업그레이드할 수 없습니다.",color:"red",italic:true}
execute if entity @s[tag=accelerator_experiment_running] run return 0

# 입자가속기는 현대(문명 단계 8) 이후에 처음 가동할 수 있다.
execute unless entity @a[tag=accelerator_experiment_running] if score #GLOBAL accelerator_level matches 0 unless score #overworld civilization_age matches 8.. run title @s actionbar {text:"현대에 도달해야 입자가속기를 가동할 수 있습니다.",color:"red",italic:true}
execute if score #GLOBAL accelerator_level matches 0 unless score #overworld civilization_age matches 8.. run return 0

execute unless entity @a[tag=accelerator_experiment_running] if score #GLOBAL accelerator_level matches 4.. run title @s actionbar {text:"입자가속기는 이미 최대 레벨입니다.",color:"red",italic:true}
...
# Lv.2부터는 보호막에서 회수한 깨진 양자 얽힘 파편도 요구한다.
# 일반 비용은 충분하지만 이 파편만 부족한 경우에도 흑요석 1개로 업그레이드 전체를 대체할 수 있다.
...
# 추가 재료 때문에 실패했더라도 흑요석을 보유했다면 전체 비용 대체로 전환한다.
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #accelerator_cost_ok tmp matches 1 run title @s actionbar {text:"업그레이드 재료가 부족합니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] run title @s actionbar [{text:"",italic:false},{text:"입자가속기 Lv.",color:"aqua"},{score:{name:"#GLOBAL",objective:"accelerator_level"},color:"white"},{text:" 업그레이드 완료",color:"aqua"}]
