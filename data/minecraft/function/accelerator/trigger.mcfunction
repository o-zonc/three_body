...
execute unless entity @a[tag=accelerator_experiment_running] if score #GLOBAL accelerator_disabled matches 1 if score @s accelerator_trigger matches 1 run title @s actionbar {text:"입자가속기가 손상되어 업그레이드할 수 없습니다.",color:"red",italic:true}
execute unless entity @a[tag=accelerator_experiment_running] if score #GLOBAL accelerator_disabled matches 1 if score @s accelerator_trigger matches 10..12 run title @s actionbar {text:"입자가속기가 수리 중이라 실험할 수 없습니다.",color:"red",italic:true}
