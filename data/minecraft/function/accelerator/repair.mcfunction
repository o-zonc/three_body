execute unless entity @a[tag=accelerator_experiment_running] unless score #GLOBAL accelerator_disabled matches 1 run title @s actionbar {text:"입자가속기는 정상 작동 중입니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #accelerator_repair_cost_ok tmp matches 1 run title @s actionbar {text:"수동 재가동에는 철 2,000,000, 금 50,000과 소지한 시간 2가 필요합니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] run title @s actionbar {text:"입자가속기 수동 재가동 완료",color:"green",bold:true,italic:false}
