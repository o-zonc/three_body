# 장기 고장 상태에서는 입자가속기의 생산·실험 재정비를 모두 정지한다.
...
# 입자가속기는 정상 가동 중이고 시간의 흐름이 멈추지 않았을 때만 정보를 자동 생산한다.
...
# 입자가속기가 가동되는 동안 주기적으로 저음의 공명음을 낸다.
...
# 외계 간섭은 현대(문명 단계 8)에서만 발생하며, 미래에 진입하면 영구적으로 종료된다.
# 극야(three_body:polar, current_dim 3)와 여명(three_body:dawn, current_dim 4)에서는 진행 중인 간섭을 정상 종료한다.
...
execute if score #GLOBAL current_dim matches 3..4 run scoreboard players set #alien_maze_paused var 0

# 미로 진입 전 상태와 남은 타이머를 보존한 채 외계 간섭을 일시정지한다.
execute if score #frozen_maze_active var matches 1 if score #GLOBAL alien_interference matches 1 run scoreboard players set #alien_maze_paused var 1
execute if score #frozen_maze_active var matches 1 if score #GLOBAL alien_interference matches 1 run scoreboard players set #GLOBAL alien_interference 0
...
execute if score #overworld civilization_age matches 9.. run scoreboard players set #alien_maze_paused var 0

# 미로 종료 후에는 최초 시작 연출 없이 보존한 간섭 상태만 복구한다.
execute unless score #GLOBAL current_dim matches 3..4 unless score #frozen_maze_active var matches 1 if score #overworld civilization_age matches 8 if score #alien_maze_paused var matches 1 run scoreboard players set #GLOBAL alien_interference 1
execute if score #GLOBAL alien_interference matches 1 if score #alien_maze_paused var matches 1 run scoreboard players set #alien_maze_paused var 0
...
# 보호막 상태 변경은 여기 한 곳에서만 수행한다.
# 미리 충전한 보호막은 외계 간섭이 시작될 때까지 보존한다.
# 간섭 차단 중에는 5분(6000틱)마다 플레이어가 소지한 시간 1개를 유지비로 사용한다.
