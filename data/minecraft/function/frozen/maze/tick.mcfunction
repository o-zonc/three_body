# 얼어붙은 세계 지하 미로 이벤트
# 복제된 미로의 플레이어 이동 높이 Y=52..59를 기준으로 진입과 중앙 도달을 판정한다.
# 다른 방식으로 차원을 떠난 경우에도 이벤트 상태가 남지 않게 정리한다.
...
# 긴급탈출장치는 별도 interaction 없이 왼손에 드는 즉시 사용한다.
...
# -34..34 정사각형 미로 내부에 처음 진입하면 새 이벤트를 시작한다.
