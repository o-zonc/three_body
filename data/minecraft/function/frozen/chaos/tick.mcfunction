# 얼어붙은 세계 난세기 환경 패널티
# 비보호 플레이어는 장소와 무관하게 채굴 피로 I을 받고, 실외에서는 점차 빙결된다.
# 보온 물약을 마시면 현재 난세기 동안 두 패널티를 모두 받지 않는다.

# 엔딩 중에는 상태와 남은 진행을 그대로 보존한다.
execute if entity @a[tag=ending_active] run return 0

# 보온 물약을 마신 뒤 남은 유리병 1개를 회수하고 보호 태그를 부여한다.
...
# 다른 플레이어가 얼어붙은 세계에 남아 있어도 차원을 떠난 당사자는 즉시 정리한다.
...
# 현재 난세기 처리 대상만 추적해 종료 시 불필요한 전체 플레이어 초기화를 피한다.
tag @a[tag=player,nbt={Dimension:"minecraft:frozen"}] add frozen_chaos_tracking

# 채굴 피로는 실내·지하·미로를 포함한 모든 비보호 플레이어에게 적용한다.
...
# can_see_sky 판정은 플레이어당 한 번만 수행해 이번 tick의 실외 노출 태그로 사용한다.
