...
# dev는 현재 얼어붙은 차원에 있는 경우만 막고, 그 외 차원 제한은 모두 우회합니다.
execute if entity @s[tag=dev,nbt={Dimension:"minecraft:frozen"}] run return run function mover/restricted/common
execute if entity @s[tag=dev] run return run function frozen/move
...
# dev 태그 플레이어는 위에서 먼저 이동하므로 소모되지 않습니다.
execute unless entity @s[tag=dev] unless entity @s[nbt={Dimension:"minecraft:polarnight"}] unless entity @s[nbt={Dimension:"minecraft:frozen"}] run clear @s minecraft:light_blue_dye[minecraft:custom_data~{frozen_move:1b}] 1
