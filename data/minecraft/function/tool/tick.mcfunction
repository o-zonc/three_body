...
# 새로 접속했거나 도구를 잃은 플레이어에게 현재 단계 도구를 복구한다.
execute as @a[tag=player,gamemode=adventure] at @s \
  unless items entity @s container.* *[minecraft:custom_data~{tool:{}}] \
  unless items entity @s weapon.offhand *[minecraft:custom_data~{tool:{}}] \
  unless items entity @s player.cursor *[minecraft:custom_data~{tool:{}}] \
  unless items entity @s player.crafting.* *[minecraft:custom_data~{tool:{}}] \
  unless entity @e[type=minecraft:item,tag=dropped_resource_tool,distance=..3] \
    run function tool/give_current
