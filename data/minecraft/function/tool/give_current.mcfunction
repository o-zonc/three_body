...
# 임시 아이템에 저장소의 컴포넌트를 직접 복사한다.
# 문자열 매크로를 사용하지 않아 custom_name과 item_model의 따옴표가 유실되지 않는다.
...
# 노랑/파랑은 첫 입자가속기 실험 이후에만 can_break 목록에 노출한다.
...
# 빈 슬롯이 없다면 기존 아이템을 덮어쓰지 않고 발밑에 드롭한다.
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #tool_give_success tmp matches 1 run title @s actionbar {text:"인벤토리가 가득 차 채굴 도구를 발밑에 떨어뜨렸습니다.",color:"yellow",italic:true}
