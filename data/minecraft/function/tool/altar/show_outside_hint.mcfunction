...
execute if data storage data tmp.tool.now.cost run title @s actionbar ["",{text:"하늘의 힘은 천공의 제단 위에서만 응답합니다.",color:"aqua",italic:true},{text:" 다음 업그레이드: ",color:"gray",italic:true},{storage:"data",nbt:"tmp.cost_text.tool_upgrade.text",interpret:true,italic:true}]
execute unless data storage data tmp.tool.now.cost run title @s actionbar {text:"하늘의 힘은 천공의 제단 위에서만 응답합니다.",color:"aqua",italic:true}
