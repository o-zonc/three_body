...
execute unless entity @a[tag=accelerator_experiment_running] unless data storage data tmp.tool.now.cost run title @s actionbar {text:"더 이상 업그레이드 할 수 없습니다.",color:"red",italic:true}
...
execute unless entity @a[tag=accelerator_experiment_running] if score #tool_upgrade_cost_check tmp matches 1 run title @s actionbar [{text:"",italic:false},{text:"하늘의 힘이 깃들었습니다  ",color:"aqua"},{storage:"data",nbt:"tmp.tool.now.components.custom_name"}]
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #tool_upgrade_cost_check tmp matches 1 run title @s actionbar {text:"재료가 부족합니다.",color:"red",italic:true}
