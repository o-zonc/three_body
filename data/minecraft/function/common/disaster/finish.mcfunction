...
# --- 멸망한 이전 차원의 상태 초기화 ---
# 이 함수가 호출되는 시점은 120틱의 멸망 연출이 끝난 뒤이다.
# current_dim은 아직 '멸망한 이전 차원'을 가리키므로,
# 다음 차원으로 이동하기 전에 해당 차원의 state만 0으로 되돌린다.
...
# 다음 차원 이동이 끝난 뒤 최신 시대·위치로 구성한 사이드바를 다시 표시한다.
...
# 이번 멸망에서 사용한 수동 정지 기록은 다음 차원 이동에 남기지 않는다.
