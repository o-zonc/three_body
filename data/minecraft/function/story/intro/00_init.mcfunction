# 인트로 0단계 - 초기화
# 이 함수는 map load 시 tick.mcfunction에 의해 딱 한 번만 호출된다.
...
execute if entity @e[tag=dev] run return run function story/intro/dev
...
# x y z 값과 Rotation(yaw pitch)을 그대로 옮겨 적으면 된다.
# yaw 0 = 남쪽을 보는 상태로 맞춰두면 아래 텍스트 회전값과 잘 맞는다.
...
# 뒤쪽을 채우는 검은 배경판
# transformation 행렬은 [가로scale,0,0,가로이동, 0,세로scale,0,세로이동, 0,0,두께scale,두께이동, 0,0,0,1] 순서이다.
