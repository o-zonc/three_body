# 인트로 마무리 - 플레이어 조작 복구
# 20초간의 낙하가 끝난 후 플레이어의 조작 권한을 복구한다.

# 플레이어의 게임 모드를 '모험'으로 되돌린다.
gamemode adventure @a

# 실제 월드 시간/날씨와 차원 전환 시스템을 함께 재개한다.
# dev 인트로 스킵은 96_end를 거치지 않으므로 여기서도 반드시 해제해야 한다.
time resume
gamerule advance_time true
gamerule advance_weather false
scoreboard players set #GLOBAL era_paused 0

# 낙하에 사용된 아머 스탠드를 제거하고 시작 발전과제를 부여한다.
...
# 본편 조작이 풀린 순간 이번 전체 회차의 외계 자원 등장 시점을 한 번만 정한다.
...
# 예약 함수에는 플레이어 실행 주체가 없으므로, 각 플레이어 문맥에서 도구를 지급한다.
function tool/refresh_all

# 개발자는 게임 시작 시 커스텀 흑요석 한 세트를 추가로 받는다.
