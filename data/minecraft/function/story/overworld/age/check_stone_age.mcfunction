# 목기 시대에 돌 해금에 필요한 자원 조건을 달성하면 석기 시대로 진입한다.
# unlock 값과 실제 해금 동작은 읽거나 호출하지 않는다.
execute unless score #overworld civilization_age matches 1 run return 0
...
execute as @a[tag=player,limit=1] at @s run function story/overworld/age/20_stone_age
return 1
