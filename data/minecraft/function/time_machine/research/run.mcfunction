...
execute if score #time_machine_research_total tmp matches 12.. run title @s actionbar {text:"타임머신 연구가 이미 완료되었습니다.",color:"green",italic:true}
...
execute unless score #time_machine_cost_ok tmp matches 1 run title @s actionbar {text:"연구에 필요한 재료가 부족합니다.",color:"red",italic:true}
...
$title @s actionbar [{text:"",italic:false},{text:"$(type) 연구 완료 · 시간 생산 ",color:"light_purple"},{score:{name:"#time_machine_amount",objective:"tmp"},color:"white"},{text:"개 / ",color:"gray"},{score:{name:"#time_machine_interval",objective:"tmp"},color:"aqua"},{text:"틱",color:"gray"}]
