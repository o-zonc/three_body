...
execute unless entity @a[tag=accelerator_experiment_running] if score #time_machine_research_total tmp matches 12.. run title @s actionbar {text:"타임머신 연구가 이미 완료되었습니다.",color:"green",italic:true}
execute if score #time_machine_research_total tmp matches 12.. run return 0

# 시간은 마일스톤(4/8/12번째 연구)에서만 요구한다.
# 타임머신 연구는 일반 업그레이드와 동일하게 흑요석 1개로 전체 비용을 대체할 수 있다.
...
# 선택한 색채 자원과 깨진 양자 얽힘 파편은 공용 비용 목록 밖에 있으므로 별도로 검사한다.
...
# 색채 자원/파편 때문에 일반 결제가 실패해도 흑요석을 보유했다면 연구 전체를 흑요석 1개로 대체한다.
...
execute unless entity @a[tag=accelerator_experiment_running] unless score #time_machine_cost_ok tmp matches 1 run title @s actionbar {text:"연구에 필요한 재료가 부족합니다.",color:"red",italic:true}
...
$execute unless score #time_machine_obsidian_bypass tmp matches 1 if score #factory_recycle_level upgrade matches 4.. run function factory/recycle/refund_time_machine {type:"$(type)"}
...
# 세대 상승 시 현재 누적 연구 비율로 발전 트리 특성을 확정한다.
...
# 마일스톤에서 세대 기본 성능과 트리 보너스가 바뀌므로 한 번 더 계산한다.
function time_machine/calculate
...
$execute unless entity @a[tag=accelerator_experiment_running] run title @s actionbar [{text:"",italic:false},{text:"$(display_type) 연구 완료 · 시간 생산 ",color:"light_purple"},{score:{name:"#time_machine_amount",objective:"tmp"},color:"white"},{text:"개 / ",color:"gray"},{score:{name:"#time_machine_interval",objective:"tmp"},color:"aqua"},{text:"틱",color:"gray"}]
function time_machine/ui

# 정상 결제를 거친 마지막 연구에서 최대 단계에 처음 도달한 경우에만 실행한다.
execute if score #time_machine_research_total tmp matches 12 if score #GLOBAL time_machine_level matches 4 unless entity @s[tag=ending_seen] run function story/ending/scroll_start
