# 세대 상승 시 현재까지의 누적 연구 투자 비율로 세대 특성을 확정한다.
# 노랑 우세: 생산량 특화 / 파랑 우세: 생산 주기 특화 / 동률: 숨겨진 초록 특성
# 발전 트리 기록은 upgrade objective에 저장하여 전체 초기화의 * upgrade 0과 함께 정리된다.
...
# 세 번의 마일스톤을 모두 같은 발전 트리로 완성하면 해당 숨겨진 발전과제를 달성한다.
...
# 초기화 이후에도 자동 채굴 보상과 전체 달성 판정을 유지하도록 과거 완료 기록을 별도로 보존한다.
